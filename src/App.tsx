...
const ComplaintForm = ({ onNewComplaint }: { onNewComplaint: (data: any) => string }) => {
  ...
  try {
    // 1. جلب البيانات الحية من الفايربيز لحساب الرقم التالي
    const res = await fetch('https://rig-system-af597-default-rtdb.firebaseio.com/live_complaints.json');
    const existingComplaints = await res.json();
    
    let nextCount = 1;
    if (existingComplaints) {
      const totalComplaints = Object.keys(existingComplaints).length;
      nextCount = totalComplaints + 1;
    }

    // 2. تحويل الرقم إلى التنسيق المطلوب (RIG-2026-00001)
    // دالة padStart تضمن وجود 5 خانات للأرقام دائماً
    const formattedId = `RIG-2026-${nextCount.toString().padStart(5, '0')}`;

    // 3. تجهيز الشكوى بالكامل بالرقم الجديد
    ...
    // 4. الإرسال المباشر للفايربيز
    await fetch('https://rig-system-af597-default-rtdb.firebaseio.com/live_complaints.json', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(finalComplaint)
    });
    
    // 5. تحديث الشاشة بالرقم المنسق الجديد
    setRefNumber(formattedId);
    setSubmitted(true);
  } catch (error) {
    console.error("Error submitting:", error);
    // حل احتياطي في حال انقطع الاتصال
    const fallbackId = `RIG-2026-${Date.now().toString().slice(-5)}`;
    setRefNumber(fallbackId);
    setSubmitted(true);
  }
};
  const handleReset = () => {
...
const StatusTracker = ({ complaints }: { complaints: Complaint[] }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
   ...
   const firebaseUrl = 'https://rig-system-af597-default-rtdb.firebaseio.com/';
    ...
    if (firebaseUrl) {
      try {
        const url = firebaseUrl.endsWith('/') ? firebaseUrl : firebaseUrl + '/';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row justify-between items-center gap-8">
        ...
        <div className="flex gap-6">
          ...
          <a href="#" className="hover:text-blue-400">البوابة الإلكترونية</a>
        </div>
      </div>
  ...
  const [settings, setSettings] = useState<Settings>(() => {
    try {
      ...
      if (saved) {
        ...
        // دمج لضمان عدم وجود خصائص مفقودة تسبب انهيار
    ...
    try {
      ...
      if (savedComplaints) {
        ...
        if (Array.isArray(parsed)) {
          // تحويل المرفقات القديمة إلى الصيغة الجديدة لضمان عدم الانهيار
...
const saveToCloud = async (data: any) => {
  ...
  try {
    await fetch(`${url}complaints.json`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleNewComplaint = (formData: any) => {
    const currentYear = new Date().getFullYear();
    const prefix = `RIG-${currentYear}-`;
    
    const yearComplaints = complaints.filter(c => c.id.startsWith(prefix));
    let nextSeq = 1;
    
    if (yearComplaints.length > 0) {
      const seqs = yearComplaints.map(c => {
    ...
    // دفع الشكوى للسحاب في حال وجود رابط
    if (settings.firebaseUrl) {
      const url = settings.firebaseUrl.endsWith('/') ? settings.firebaseUrl : settings.firebaseUrl + '/';
      fetch(`${url}complaints.json`, {
        method: 'POST',
        body: JSON.stringify(newComplaint)
      }).then(() => console.log("Pushed to cloud"));
    }
  ...
  const handleUpdateStatus = (id: string, status: any, reply?: string) => {
    const updated = complaints.map(c => {
      if (c.id === id) {
        ...
        // استخدام نظام mailto لفتح تطبيق البريد مباشرة (البرنامج وليس الصفحة)
